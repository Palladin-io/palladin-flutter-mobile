...
void main() {
  ...
  test('loadMore failure surfaces loadMoreError and keeps existing entries',
      () async {
    when(() => agents.listAgents()).thenAnswer((_) async => []);
    when(() => audit.listVaultLogs('v-1',
            cursor: null, pageSize: any(named: 'pageSize')))
        .thenAnswer((_) async =>
            AuditLogPage(entries: [_entry('1', agentId: 'a-1')], nextCursor: 'c1'));
    await cubit.load();
    expect(cubit.state.entries, hasLength(1));
    expect(cubit.state.nextCursor, 'c1');

    when(() => audit.listVaultLogs('v-1',
            cursor: 'c1', pageSize: any(named: 'pageSize')))
  ...
  test('loadMore resolves the name of an agent first seen on a later page',
      () async {
    when(() => agents.listAgents())
    ...
    when(() => audit.listVaultLogs('v-1',
            cursor: 'c1', pageSize: any(named: 'pageSize')))
        .thenAnswer((_) async =>
            AuditLogPage(entries: [_entry('2', agentId: 'a-2')], nextCursor: null));
    await cubit.loadMore();
