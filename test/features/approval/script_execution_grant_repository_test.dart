...
void main() {
  ...
  test(
    'four Script references create one ScriptExecution grant POST',
    () async {
      ...
      when(
        ...
        final isScript = expected.id == scriptId;
        return CanonicalEntrySnapshot(
          entry: {
            'id': expected.id,
            'organizationId': organizationId,
            'vaultId': vaultId,
            'currentRevision': isScript ? '7' : '3',
          },
          payload: isScript
              ? {
                  'script': 'echo ok',
      ...
      when(
        () => canonical.encodeCanonicalMemberSecret(any()),
      ).thenReturn(Uint8List.fromList(utf8.encode('{"secret":true}')));
      when(
        () => packages.seal(
