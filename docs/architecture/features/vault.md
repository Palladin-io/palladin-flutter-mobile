...
- **Public website icons:** import writes only a normalized `website:<hostname>` reference into the encrypted Member projections and never waits for catalog resolution. The Entry list schedules the complete unlocked list up to the 10,000-head sync bound, splits resolution into backend pages of 500, and must never silently truncate the 501st or later hostname. UI polling remains bounded to ten attempts. A manually selected `public-asset:` or encrypted icon always wins; older rows without an icon reference derive a runtime-only `website:` fallback from their authenticated autofill domain.
