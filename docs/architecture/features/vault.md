...
- `MemberIndexPreparationService` is the session-scoped single-flight boundary shared by Dashboard search and AutoFill. Concurrent unlock consumers share one Vault-list load and one Member sync chain per Vault. The authenticated encrypted `memberVaultKey` context already present in `GET /api/vaults` is retained only in process memory and reused by the entry loader, avoiding an otherwise redundant `GET /api/vaults/{id}` per Vault. The context, copied private keys, opened VKs, and in-flight preparation generation are invalidated on lock; key copies are wiped in `finally` paths.
