...
- `MemberSyncService` consumes protocol-2 snapshot/delta pages with the frozen protocol/policy headers. Snapshot pages are streamed through a staging SQLite namespace and promoted atomically only after every page authenticates and decrypts; an interruption preserves the last complete cache. Every promoted snapshot is immediately closed with a delta starting at its `snapshotBaseSequence`, so writes committed while snapshot pages were downloading are not missed. Delta items and their applied-through sequence commit in one SQLite transaction, and `resetRequired` starts a new snapshot.
...
- `MemberIndexPreparationService` is the session-scoped single-flight boundary shared by Dashboard search and AutoFill. Concurrent unlock consumers share one Vault-list load and one Member sync chain per Vault. The authenticated encrypted `memberVaultKey` context already present in `GET /api/vaults` is retained only in process memory and reused by the entry loader, avoiding an otherwise redundant `GET /api/vaults/{id}` per Vault. If a snapshot or delta advertises a newer Member generation in its structural head, sync stops before advancing the ciphertext cache, fetches one fresh Vault detail context, and retries once; this keeps rotations completed by this or another device from turning new heads into corrupt placeholders. The context, copied private keys, opened VKs, and in-flight preparation generation are invalidated on lock; key copies are wiped in `finally` paths.
