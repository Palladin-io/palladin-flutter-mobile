...
class GrantsRemoteDatasource {
  ...
  ///
  /// Org-wide grants across every vault the caller can manage (the Approvals
  /// "history" feed). Each item carries its own `vaultId`, so no context id is
  /// applied. All filters optional. Returns one [GrantPage].
  Future<GrantPage> listOrgGrants({
    String? status,
    String? agentId,
    String? vaultId,
    String? entryId,
